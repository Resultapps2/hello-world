...
import { Container, Content, Header, Title,Left,Button, Right, Body, Icon, Card, Footer, FooterTab, Badge, DatePicker } from 'native-base';
...
class First extends Component {
  ...
  async componentDidMount() {
    ...
    await fetch('https://vip4dresult.azurewebsites.net/nine93', {
      method: 'GET',
    })
  ...
  async setDate(newDate) {
    ...
    day = String(day).length > 1 ? day : '0' + day
    month = String(month).length > 1 ? month : '0' + month
    let fullDate = 'https://vip4dresult.azurewebsites.net/nine932/'+year+month+day
  ...
  render() {
    ...
    console.log('screen shot retrun from render', this.state.uri)
    return (
      ...
      <Content>
        <View>
          <Card style={{ backgroundColor: '#f6eb13', height: 100, paddingTop: 10 }}>
            <View style={{ flexDirection: 'row', paddingHorizontal: 10 }}>
              <Left></Left>
              <Body><Text style={{ color: '#000', fontSize: 20, fontWeight: "bold" }}>{(this.state.toggle) === 0 ? "99 3PM" : (this.state.toggle) === 1 ? "99下午3点": "99 3Petang"}</Text></Body>
        ...
        <View style={{ paddingVertical: 20 }}>
           <TouchableOpacity ><Button
              onPress={() => this.screenshot(shareImageBase64) } style={{ width: '100%', backgroundColor: '#f6eb13', alignItems: 'center', justifyContent: 'center' }} >
              <Text style={{ color: '#000', fontSize: 20, fontWeight: "bold" }}>
                {(this.state.toggle) === 0 ? "Share" : (this.state.toggle) === 1 ? "分享" : "Kongsi"}
              </Text>
            </Button>
            </TouchableOpacity> 
          </View>
