  ...
  render() {
    ...
    return (
      <Container style={{ backgroundColor: '#fff' }}>
        <Content>
          <View>
            <Card style={{ backgroundColor: '#CFD7DC', height: 105, paddingTop: 10 }}>
              <View style={{ flexDirection: 'row', paddingHorizontal: 10 }}>
                <Left style={{ flexDirection: 'row' }}><Text style={{ color: '#000', fontSize: 16 }}>{this.state.draw}</Text></Left>
                <Body><Text style={{ color: '#000', fontSize: 20, fontWeight: "bold" }}>{(this.state.toggle) === 0 ? "STC 4D" : (this.state.toggle) === 1 ? "山打根赛马会" : "STC 4D"}</Text></Body>
                <Right><Image source={require('../assets/stc.jpg')} style={{ width: 40, height: 40 }}
                /></Right>
              </View>
          ...
          <View style={{ paddingVertical: 20 }}>
            <Button onPress={() => this.screenshot(shareImageBase64) } style={{ width: '100%', backgroundColor: '#CFD7DC', alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: '#000', fontSize: 20, fontWeight: "bold" }}>{(this.state.toggle) === 0 ? "Share" : (this.state.toggle) === 1 ? "分享" : "Kongsi"}</Text></Button>
          </View>
...
export default withNavigation(Eighth)
