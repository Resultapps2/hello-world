...
class Second extends Component {
  ...
  async componentDidMount() {
    ...
    await fetch('https://vip4dresult.azurewebsites.net/nine97', {
      method: 'GET',
    })
    ...
    return (
      <View style={{ backgroundColor: '#F4F8E5' }}>
        ...
        <View style={{ paddingVertical: 20 }}>
          <Button onPress={() => this.screenshot(shareImageBase64) } style={{ width: '100%', backgroundColor: '#ED1E24', alignItems: 'center', justifyContent: 'center' }}><Text style={{ color: '#fff', fontSize: 20, fontWeight: "bold" }}>{(this.state.toggle) === 0 ? "Share" : (this.state.toggle) === 1 ? "分享" : "Kongsi"}</Text></Button>
        </View>
