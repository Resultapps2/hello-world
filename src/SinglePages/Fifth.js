      ...
      <Container style={{ backgroundColor: '#fff' }}>
        <Content>
          <View>
            <Card style={{ backgroundColor: '#CFD7DC', height: 100, paddingTop: 10 }}>
              <View style={{ flexDirection: 'row', paddingHorizontal: 10 }}>
                ...
                <Body><Text style={{ color: '#000', fontSize: 20, fontWeight: "bold" }}>{(this.state.toggle) === 0 ? "Toto" : (this.state.toggle) === 1 ? "多多" : "Toto"}</Text></Body>
                <Right><Image source={require('../assets/toto.png')} style={{ width: 40, height: 40 }}
                /></Right>
              </View>
          ...
          <View style={{ paddingVertical: 20 }}>
            <Button onPress={() => this.screenshot(shareImageBase64) } style={{ width: '100%', backgroundColor: '#CFD7DC', alignItems: 'center', justifyContent: 'center' }}><Text style={{ fontSize: 20, fontWeight: "bold", color: '#000' }}>{(this.state.toggle) === 0 ? "Share" : (this.state.toggle) === 1 ? "分享" : "Kongsi"}</Text></Button>
          </View>
...
export default withNavigation(Fifth)
