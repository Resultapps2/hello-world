...
export class MPT extends Component {
  ...
  async componentDidMount() {
    ...
    await fetch('https://vip4dresult.azurewebsites.net/nine932/'+year+month+day, {
      ...
      await fetch('https://vip4dresult.azurewebsites.net/nine972/'+year1+month1+day1, {
        method: 'GET',
      })
      .then((response) => response.json())
      .then((response) => {
  ...
  async setDate2(newDate) {
    ...
    day = String(day).length > 1 ? day : '0' + day
    month = String(month).length > 1 ? month : '0' + month
    let fullDate = 'https://vip4dresult.azurewebsites.net/nine972/'+year+month+day
    console.log('date', fullDate)
    // this.newStates(fullDate);
    await fetch(fullDate, {
      method: 'GET',
    ...
    return (
      <View>
          ...
          <View style={{ paddingVertical: 20 }}>
            <Button onPress={() => this.screenshot(shareImageBase64) } style={{ height: 50, width: '100%', backgroundColor: '#f6eb13', alignItems: 'center', justifyContent: 'center' }}><Text style={{ fontSize: 20, color: '#000',fontWeight:'bold' }}>{(this.state.toggle) === 0 ? "Share" : (this.state.toggle) === 1 ? "分享" : "Kongsi"}</Text></Button>
          </View>
