...
export class S extends Component {
  ...
  async componentDidMount() {
    ...
    await fetch('https://vip4dresult.azurewebsites.net/sabah88', {
      method: 'GET',
    })
      ...
      .then((response) => {
        ...
        await fetch('https://vip4dresult.azurewebsites.net/stc', {
          ...
          await fetch('https://vip4dresult.azurewebsites.net/cashSweep', {
        method: 'GET',
      })
        .then((response) => response.json())
        .then((response) => {
  ...
  async setDate1(newDate) {
    ...
    let fullDate = 'https://vip4dresult.azurewebsites.net/sabah882/'+year+'-'+month+'-'+day
  ...
  async setDate2(newDate) {
    ...
    day = String(day).length > 1 ? day : '0' + day
    month = String(month).length > 1 ? month : '0' + month
    let fullDate = 'https://vip4dresult.azurewebsites.net/stc2/'+year+'-'+month+'-'+day
    console.log('date', fullDate)
    await fetch(fullDate, {
      method: 'GET',
    })
  ...
  async setDate3(newDate) {
    ...
    day = String(day).length > 1 ? day : '0' + day
    month = String(month).length > 1 ? month : '0' + month
    let fullDate = 'https://vip4dresult.azurewebsites.net/cashSweep2/'+year+'-'+month+'-'+day
    console.log('date', fullDate)
    // this.newStates(fullDate);
    await fetch(fullDate, {
      method: 'GET',
    ...
    return (
      <View>
          <View style={{ paddingTop: 10, paddingHorizontal: 10, flexDirection: 'row', justifyContent: 'space-evenly' }}>
              ...
              <Card style={{ backgroundColor: '#CFD7DC', height: 200, width: 110, paddingTop: 10, alignItems: 'center' }}>
                <View >
                  ...
                  /></View>
                <View>
                  <Text style={styles.textStyle}>{(this.state.toggle) === 0 ? "Sabah 88" : (this.state.toggle) === 1 ? "沙巴萬字" : "Sabah 88"}</Text>
                </View>
              ...
              <View style={styles.container}>
                <Table style={{ flexDirection: 'row' }} borderStyle={{ borderWidth: 4, borderColor: '#c5cbd6' }}>
                  <TableWrapper style={{ width: 110 }}>
                    <Cell data={(this.state.toggle) === 0 ? "Consolation" : (this.state.toggle) === 1 ? "安慰獎": "Saguhat"} style={{ width: 110, height: 40, backgroundColor: '#CFD7DC' }} textStyle={{ color: '#000', margin: 6, alignSelf: 'center',fontSize:16,fontWeight:'bold' }} />
                    <TableWrapper style={{ flexDirection: 'row',height:550}}>
              ...
              <Card style={{ backgroundColor: '#CFD7DC', height: 200, width: 110, paddingTop: 10, alignItems: 'center' }}>
                <View >
                  ...
                  /></View>
                <View>
                  <Text style={styles.textStyle}>{(this.state.toggle) === 0 ? "STC 4D" : (this.state.toggle) === 1 ? "山打根赛马会" : "STC 4D"}</Text>
                </View>
                <View style={{ flexDirection: 'row',paddingLeft:6  }}>
                  <Icon name='calendar' style={{ color: '#000', paddingTop: 10, fontSize: 18 }} />
                  <DatePicker
              ...
              <Card style={{ backgroundColor: '#CFD7DC', height: 200, width: 110, paddingTop: 10, alignItems: 'center' }}>
                <View >
                  ...
                  /></View>
                <View>
                  <Text style={styles.textStyle}>{(this.state.toggle) === 0 ? "Cash Sweep" : (this.state.toggle) === 1 ? "砂勞越大萬" : "Cash Sweep"}</Text>
                </View>
                <View style={{ flexDirection: 'row',paddingLeft:6  }}>
                  <Icon name='calendar' style={{ color: '#000', paddingTop: 10, fontSize: 18 }} />
                  <DatePicker
          ...
          <View style={{ paddingVertical: 20 }}>
            <Button onPress={() => this.screenshot(shareImageBase64) } style={{ height: 50, width: '100%', backgroundColor: '#CFD7DC', alignItems: 'center', justifyContent: 'center' }}><Text style={{ fontSize: 20, color: '#000',fontWeight:'bold' }}>{(this.state.toggle) === 0 ? "Share" : (this.state.toggle) === 1 ? "分享" : "Kongsi"}</Text></Button>
          </View>
